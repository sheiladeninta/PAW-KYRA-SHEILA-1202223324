...
if(isset($_GET['search']) ? $_GET['search'] : ''){
    ...
    // Hint : Gunakan strlen()
    if (strlen($search) < 3) {
        echo "<script>alert('Input search minimal harus 3 karakter.')</script>";
    }
    // (3.) Validasi Input jika search input tidak alphanumeric
    // Hint : Gunakan preg_match()
    $pattern = "/^[a-zA-Z0-9\s]+$/";
    if (preg_match($pattern, $search)) {
        echo "<script>alert('Input valid: ')</script>" . $search;
    } else {
        echo "<script>alert('Input tidak valid. Harap masukkan hanya huruf, angka, dan spasi.')</script>";
    }
} else {
    $search = '';
}
